...
private Queue<CombatEvent> combatEvents = new LinkedBlockingQueue<CombatEvent>();
	...
	{	
		...
		{	
			...
			{
				timer = 0;
				if(level.isPlayerEntitySelected())
				{
					int dmg = 0;
					...
					if(dmg > 0)
					{
						...
						{
							level.getParticleEntities().add(new Particle(this.target1.getPixel64X() + level.getPlayer().xScroll, this.target1.getPixel64Y() + level.getPlayer().yScroll, 30));
						}
					}
					endAttack(dmg*-1);
				}
			...
			else
			{
				...
				//Check to the right
				if(this.getCenter64X()+1 == level.getPlayerArmyEntities().get(i).getCenter64X() && this.getCenter64Y() == level.getPlayerArmyEntities().get(i).getCenter64Y())
				{
					setImage(Assets.enemAnimRightAtk.getImage());
				}
				//check to the left
				else if(this.getCenter64X()-1 == level.getPlayerArmyEntities().get(i).getCenter64X() && this.getCenter64Y() == level.getPlayerArmyEntities().get(i).getCenter64Y())
				{	
					setImage(Assets.enemAnimLeftAtk.getImage());
				}
				// check above
				else if(this.getCenter64Y()-1 == level.getPlayerArmyEntities().get(i).getCenter64Y() && this.getCenter64X() == level.getPlayerArmyEntities().get(i).getCenter64X())
				{
		...
		else 
		{
			...
			if (!searched && target1 != null) 
			{
				...
				pathToTarget = new LinkedBlockingDeque<Tile>( breadthFirstSearch(level.getArmyMovementTileMap()));
	...
	********************************************************/
	...
	public boolean isSpaceOccupied(Tile cur)
	{
		...
		if (cur != null)
		{
			...
			{
				if(level.getEnemyArmyEntities().get(i).getCenter64X() == cur.getTileX() && level.getEnemyArmyEntities().get(i).getCenter64Y() == cur.getTileY())
				{
			...
			if(isSpaceOccupied == false)
			{
				...
				{
					if(level.getPlayerArmyEntities().get(i).getCenter64X() == cur.getTileX() && level.getPlayerArmyEntities().get(i).getCenter64Y() == cur.getTileY())
					{
						isSpaceOccupied = true;
						pathToTarget = null;
...
/*private boolean getCollision()
{
	...
	{
		for(int i = 0; i < level.getDroppedResourceEntities().size(); i++)
		{
			if(level.getDroppedResourceEntities().get(i).getCenter64X() == getCenter64X() && level.getDroppedResourceEntities().get(i).getCenter64Y() == getCenter64Y())
			{				
					if(level.getDroppedResourceEntities().get(i).getResourceType().equals(ResourceType.Food))
					{
						//System.out.println("RECIEVED FOOD");
						level.getDroppedResourceEntities().get(i).setVisible(false);
					}
					else if(level.getDroppedResourceEntities().get(i).getResourceType().equals(ResourceType.Gold)) 
					{
	...
	public void die() 
	{
		...
		{
			// add resource
			Resource gold = new Resource(this.getCenter64X()*2, this.getCenter64Y()*2, level, UnitType.Melee, ResourceType.Gold);
			//System.out.println("meat X:" + this.getCenter64X() + " Y : " + this.getCenter64Y());
			gold.setImage(Sprite.goldResource.getLotus());
			level.getDroppedResourceEntities().add(gold);
		}
		//else if (i <= 5 && i > 2)
		else
		{
			// add resource
			Resource meat = new Resource(this.getCenter64X()*2, this.getCenter64Y()*2, level, UnitType.Melee, ResourceType.Food);
			//System.out.println("meat X:" + this.getCenter64X() + " Y : " + this.getCenter64Y());
			meat.setImage(Sprite.meatResource.getLotus());
			level.getDroppedResourceEntities().add(meat);
		}
		
		
			level.getPlayerArmyEntities().get(this.targetLinkPos).xp += 4;
	...
	public void endAttack(int dmg) 
	{
		...
		//GameManager.level.getEnemyArmyEntities().get(this.targetLinkPos).setPixel64X(GameManager.level.getEnemyArmyEntities().get(this.targetLinkPos).getPixel64X()-24);	
			...
			level.getPlayerArmyEntities().get(i).setTarget(new Entity(this));;

			level.getPlayerArmyEntities().get(i).addHealth(dmg);
			setCombatMode(false);
			walking = false;
	...
	public void endPlayerInitiatedAttack(int dmg) 
	{
		...
		//GameManager.level.getEnemyArmyEntities().get(this.targetLinkPos).setPixel64X(GameManager.level.getEnemyArmyEntities().get(this.targetLinkPos).getPixel64X()-24);	
			...
			if(level.getPlayerArmyEntities().get(i).unitClass == UnitClass.Archer)
			{
				if(direction == 1)
				{
	...
	***************************/
	public int calculateDmg() 
	{
		...
		{	
			...
			return (r * atkRating) - level.getPlayerArmyEntities().get(targetLinkPos).getDefRating();		
		}
	}
	...
	{
		//  Right
		if(level.getPlayerArmyEntities().get(i).getCenter64X() + 1 == this.center64X && level.getPlayerArmyEntities().get(i).getCenter64Y() == this.center64Y)
		{
			return 1;
		}
		// Left
		else if(level.getPlayerArmyEntities().get(i).getCenter64X() - 1 == this.center64X && level.getPlayerArmyEntities().get(i).getCenter64Y() == this.center64Y)
		{
			return 3;
		}
		// Down
		else if(level.getPlayerArmyEntities().get(i).getCenter64X() == this.center64X && level.getPlayerArmyEntities().get(i).getCenter64Y() - 1 == this.center64Y)
		{
			return 0;
		}
		// Up
	 ...
	 public int getTargetsDmgBasedOnDir(int dir, int dirOfTarget, int dmg)
	 {
		 switch(dirOfTarget)
		 {
		 	case 0: // target is below
		 		...
		 		if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.DOWN)
		 		{
		 			return (int) ((int) dmg + (.5 * dmg));
		 		}
		 		// check for target facing either side, inflict 25% of bonus dmg
		 		else if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.LEFT || level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.RIGHT)
		 		{
		 			return (int) ((int) dmg + (.25 * dmg));
		 		}
		 		// check for target facing forward, no bonus dmg.
		 		else if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.UP)
		 		{
		 			return dmg;
		 	...
		 	case 1: // target is to the right
		 		...
		 		if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.RIGHT)
		 		{
		 			return (int) ((int) dmg + (.5 * dmg));
		 		}
		 		// check for target facing down or forward, inflict 25 % bonus dmg
		 		else if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.DOWN || level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.UP)
		 		{
		 			return (int) ((int) dmg + (.25 * dmg));
		 		}
		 		// check for target facing left, no bonus dmg
		 		else if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.LEFT)
		 		{
		 			return dmg;
		 		}
		 		break;
		 	...
		 	case 2: // target is above
		 		...
		 		if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.UP)
		 		{
		 			return (int) ((int) dmg + (.5 * dmg));
		 		}
		 		...
		 		else if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.DOWN)
		 		{
		 			return dmg;
		 		}
		 		break;
		 	...
		 	case 3: // target is to the left
		 		...
		 		if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.LEFT)
		 		{
		 			return (int) ((int) dmg + (.5 * dmg));
		 		}
		 		// check for target facing down or forward, inflict 25 % bonus dmg
		 		...
		 		else if(level.getPlayerArmyEntities().get(this.targetLinkPos).getDirection() == Direction.RIGHT)
		 		{
		 			return dmg;
		 		}
		 		break;
	...
	public void PRESET_TARGET()
	{
		...
		{
			setTarget(level.getPlayerCityEntities().get(0));
	...
	public void WANDER() 
	{
		...
		int i = r.nextInt(12);
		int rX = r.nextInt(level.getAMMW());
		int rY = r.nextInt(level.getAMMH());
	...
	public void MOVE_TO_TARGET()
	{
		...
		for(int ii = 0; ii < level.getPlayerArmyEntities().size(); ii++)
		{			
			if(this.getCenter64X() + 1 == level.getPlayerArmyEntities().get(ii).getCenter64X() && this.getCenter64Y() == level.getPlayerArmyEntities().get(ii).getCenter64Y())  
			{
				//System.out.println("ATTACK");
				level.getPlayerArmyEntities().get(ii).setPosInLink(ii);
				setTarget(level.getPlayerArmyEntities().get(ii));
				getBrain().setState(UnitState.ATTACK); 
				ATTACK();

				break;
			}
			else if(this.getCenter64X() - 1 == level.getPlayerArmyEntities().get(ii).getCenter64X() && this.getCenter64Y() == level.getPlayerArmyEntities().get(ii).getCenter64Y())
			{
				//System.out.println("ATTACK");
				level.getPlayerArmyEntities().get(ii).setPosInLink(ii);
				setTarget(level.getPlayerArmyEntities().get(ii));
			...
			else if(this.getCenter64X() == level.getPlayerArmyEntities().get(ii).getCenter64X() && this.getCenter64Y() +1 == level.getPlayerArmyEntities().get(ii).getCenter64Y()  )
			{
				//System.out.println("ATTACK");
				level.getPlayerArmyEntities().get(ii).setPosInLink(ii);
				setTarget(level.getPlayerArmyEntities().get(ii));
			...
			else if(this.getCenter64X()-1 == level.getPlayerArmyEntities().get(ii).getCenter64X() && this.getCenter64Y() -1 == level.getPlayerArmyEntities().get(ii).getCenter64Y()  )
			{
				//System.out.println("ATTACK");
				level.getPlayerArmyEntities().get(ii).setPosInLink(ii);
				setTarget(level.getPlayerArmyEntities().get(ii));
		...
		for( int ii = 0; ii < level.getPlayerCityEntities().size(); ii++)
		{
			if(level.getPlayerCityEntities().get(ii).getCenter64X()+1 == this.getCenter64X() && level.getPlayerCityEntities().get(ii).getCenter64Y() == this.getCenter64Y())
			{
				setTarget(level.getPlayerCityEntities().get(ii));
				ATTACK();
				getBrain().setState(UnitState.ATTACK); 
				break;
							
			}
			else if(level.getPlayerCityEntities().get(ii).getCenter64X()-1 == this.getCenter64X() && level.getPlayerCityEntities().get(ii).getCenter64Y() == this.getCenter64Y())
			{
				setTarget(level.getPlayerCityEntities().get(ii));
				ATTACK();
				getBrain().setState(UnitState.ATTACK); 
			...
			else if(level.getPlayerCityEntities().get(ii).getCenter64X() == this.getCenter64X() && level.getPlayerCityEntities().get(ii).getCenter64Y()+1 == this.getCenter64Y())
			{
				setTarget(level.getPlayerCityEntities().get(ii));
				ATTACK();
				getBrain().setState(UnitState.ATTACK); 
		...
		if(target1 != null)
		{
			if(getTarget().getEntityType() != EntityType.Void)
			{
				...
				switch(t)
				{
					...
					for(int i = 0; i < level.getEnemyArmyEntities().size(); i++)
					{
						if(level.getEnemyArmyEntities().get(i).getCenter64X() == this.getCenter64X() && level.getEnemyArmyEntities().get(i).getCenter64Y() == this.getCenter64Y())
						{
							for(int j = 0; j < level.getPlayerArmyEntities().size(); j++)
							{
								if(level.getPlayerArmyEntities().get(j).getCenter64X() == this.target1.getCenter64X() && level.getPlayerArmyEntities().get(j).getCenter64Y() == this.target1.getCenter64Y())
								{
									if(target1 != null)
									{
										level.getCombatEvents().add(new CombatEvent(i, j));
									}
								//System.out.println("***********************************************************");
								//break;
								}
					...
					case P_Building:
						...
						if(this.gridMoves > 0)
						{
							if(level.getPlayerCityEntities().size() != 0 )
							{	
								level.getPlayerCityEntities().get(getTarget().getPosInLink()).addHealth(-5);
