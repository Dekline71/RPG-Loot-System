...
public class Camera 
{
	...
	/*public void renderProjectileEntity(int xPos, int yPos, Projectile p, Level level) 
	{
		Graphics g = game.getCanvas().getBufferStrategy().getDrawGraphics();
		int mapX = (xPos * 64);
		int mapY = (yPos * 64);
	...
	public void renderParticleEntity(int xPos, int yPos, Particle p, Level level)
	{
		Graphics g = game.getCanvas().getBufferStrategy().getDrawGraphics();
		int mapX = (xPos * 64);
		int mapY = (yPos * 64);
		...
		else if (!level.getPlayer().isPanelOn())
		{
			g.drawImage( p.getImage(), (p.getPixel64X()) , (p.getPixel64Y()), null );
		}
	}
	
	public void render64Entity(int xPos, int yPos, Entity e, Level level)
	{
		//xPos -= s.xOffset;
		//yPos -= s.yOffset;
		...
		// draw entity/sprite if between the scroll offset range
		if (this.getxOffset() <= xabsolute && this.getyOffset() <= yabsolute && (mapX - mapXoffset) - this.getxOffset() > 0 && (mapY - mapYoffset) - this.getyOffset() > 0 && level.getPlayer().isPanelOn())
		{
			g.drawImage((Image) e.getImage(), ((mapX-mapXoffset)-this.getxOffset()) + 187, ((mapY-mapYoffset)-this.getyOffset()), null);
		}
		else if (this.getxOffset() <= xabsolute && this.getyOffset() <= yabsolute && (mapX - mapXoffset) - this.getxOffset() > -16 && (mapY - mapYoffset) - this.getyOffset() > -16 && !level.getPlayer().isPanelOn())
		{
